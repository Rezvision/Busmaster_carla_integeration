...
bool InitUDP()
{
    ...
    if (!hWs2_32) {
        Trace("Speed ECU: Failed to load ws2_32.dll");
        return false;
    }
    ...
    if (!pfnWSAStartup || !pfnSocket || !pfnBind || !pfnRecvfrom) {
        Trace("Speed ECU: Failed to get Winsock functions");
        FreeLibrary(hWs2_32);
        return false;
    }
    ...
    if (pfnWSAStartup(0x0202, wsaData) != 0) {
        Trace("Speed ECU: WSAStartup failed");
        FreeLibrary(hWs2_32);
        return false;
    }
    ...
    if (!udpSocket || udpSocket == (void*)-1) {
        Trace("Speed ECU: Socket creation failed");
        pfnWSACleanup();
        FreeLibrary(hWs2_32);
        return false;
    }
    ...
    if (pfnIoctlsocket(udpSocket, 0x8004667E, &mode) != 0) {
        Trace("Speed ECU: Failed to set non-blocking mode");
    ...
    addr.port = pfnHtons(5100);
    addr.addr = 0;    // INADDR_ANY
    
    if (pfnBind(udpSocket, &addr, sizeof(addr)) != 0) {
        Trace("Speed ECU: Bind to port 5100 failed");
        pfnClosesocket(udpSocket);
        pfnWSACleanup();
        FreeLibrary(hWs2_32);
        return false;
    ...
    Trace("Speed ECU: UDP socket bound to port 5100, waiting for CARLA data");
    useSimulation = false;
    return true;
}
...
void OnDLL_Load()
{
    Trace("Speed ECU: Loading...");
    
    if (!InitUDP()) {
        Trace("Speed ECU: Failed to initialize UDP, running in simulation mode");
        useSimulation = true;
    } else {
        Trace("Speed ECU: Successfully initialized UDP receiver on port 5100");
    }
}
...
void OnTimer_OnTimer_Tran_10()
{
    ...
    if (!useSimulation && udpSocket && pfnRecvfrom) {
        ...
        if (received >= 4) {
            ...
            speed = NetworkToHostFloat((unsigned char*)buffer);
            lastSpeed = speed;
            dataReceived = true;
            
            // Log the received data periodically
            if (messageCount % 20 == 0) {
                Trace("Speed ECU: Received %.2f km/h from CARLA", speed);
            }
        }
    }
    
    // If no data received, use last known speed or simulation
    if (!dataReceived) {
        if (!useSimulation) {
            // Use last received speed from CARLA
            speed = lastSpeed;
        } else {
            // Simulation mode - generate test data
            static int simCounter = 0;
            simCounter++;
            speed = 50.0f + 30.0f * sin(simCounter * 0.01f);
        }
    }
    ...
    // Pack speed as little-endian (native x86 format) for CAN
    memcpy(speedMsg.data, &speed, 4);
    ...
    messageCount++;
    if (messageCount % 20 == 0) {
        Trace("Speed ECU: TX CAN 0x%03X Speed=%.2f km/h %s", 
              speedMsg.id, speed, 
