...
bool InitUDP()
{
    ...
    if (!hWs2_32) {
        Trace("brake ECU: Failed to load ws2_32.dll");
        return false;
    }
    ...
    if (!pfnWSAStartup || !pfnSocket || !pfnBind || !pfnRecvfrom) {
        Trace("brake ECU: Failed to get Winsock functions");
        FreeLibrary(hWs2_32);
        return false;
    }
    ...
    if (pfnWSAStartup(0x0202, wsaData) != 0) {
        Trace("brake ECU: WSAStartup failed");
        FreeLibrary(hWs2_32);
        return false;
    }
    ...
    if (!udpSocket || udpSocket == (void*)-1) {
        Trace("brake ECU: Socket creation failed");
        pfnWSACleanup();
        FreeLibrary(hWs2_32);
        return false;
    }
    ...
    if (pfnIoctlsocket(udpSocket, 0x8004667E, &mode) != 0) {
        Trace("brake ECU: Failed to set non-blocking mode");
    ...
    addr.port = pfnHtons(5103);
    addr.addr = 0;    // INADDR_ANY
    
    if (pfnBind(udpSocket, &addr, sizeof(addr)) != 0) {
        Trace("brake ECU: Bind to port 5100 failed");
        pfnClosesocket(udpSocket);
        pfnWSACleanup();
        FreeLibrary(hWs2_32);
        return false;
    ...
    Trace("brake ECU: UDP socket bound to port 5101, waiting for CARLA data");
    useSimulation = false;
    return true;
}
...
void OnDLL_Load()
{
    Trace("brake ECU: Loading...");
    
    if (!InitUDP()) {
        Trace("brake ECU: Failed to initialize UDP, running in simulation mode");
        useSimulation = true;
    } else {
        Trace("brake ECU: Successfully initialized UDP receiver on port 5101");
    }
}
...
void OnTimer_OnTimer_Tran_10()
{
    ...
    if (!useSimulation && udpSocket && pfnRecvfrom) {
        ...
        if (received >= 4) {
            ...
            brake = NetworkToHostFloat((unsigned char*)buffer);
            lastbrake = brake;
            dataReceived = true;
            
            // Log the received data periodically
            if (messageCount % 20 == 0) {
                Trace("brake ECU: Received %.2f from CARLA", brake);
            }
        }
    }
    
    // If no data received, use last known brake or simulation
    if (!dataReceived) {
        if (!useSimulation) {
            // Use last received brake from CARLA
            brake = lastbrake;
        } else {
            // Simulation mode - generate test data
            static int simCounter = 0;
            simCounter++;
            brake = 50.0f + 30.0f * sin(simCounter * 0.01f);
        }
    }
    ...
    // Pack brake as little-endian (native x86 format) for CAN
    memcpy(brakeMsg.data, &brake, 4);
    ...
    messageCount++;
    if (messageCount % 20 == 0) {
        Trace("brake ECU: TX CAN 0x%03X brake=%.2f  %s", 
              brakeMsg.id, brake, 
