...
GCC_EXTERN void GCC_EXPORT OnTimer_brakeTransmit_10();
...
bool InitUDP()
{
    ...
    if (pfnBind(udpSocket, &addr, sizeof(addr)) != 0) {
        Trace("brake ECU: Bind to port 5103 failed");
        pfnClosesocket(udpSocket);
        pfnWSACleanup();
        FreeLibrary(hWs2_32);
        return false;
    ...
    Trace("brake ECU: UDP socket bound to port 5103");
    return true;
}
...
void OnDLL_Load()
{
    ...
    if (!InitUDP()) {
        Trace("brake ECU: ERROR - Failed to initialize UDP!");
    } else {
        Trace("brake ECU: Ready, waiting for CARLA data on port 5103");
    }
}
...
/* Start BUSMASTER generated function - OnTimer_brakeTransmit_10 */
void OnTimer_brakeTransmit_10()
{
    ...
    if (received >= 4) {
        ...
        float brake = NetworkToHostFloat((unsigned char*)buffer);
        
        /* Set signal value - copy float bytes directly */
        UINT32 brakeValue;
        memcpy(&brakeValue, &brake, 4);
        brakeMsg.brake_current = brakeValue;
        
        /* Send CAN message */
        SendMsg(brakeMsg);
        
        messageCount++;
        Trace("brake ECU: RX %.2f  -> TX CAN ID=0x126 Data=[%02X %02X %02X %02X]", 
              brake,
              (brakeValue) & 0xFF,
              (brakeValue >> 8) & 0xFF,
              (brakeValue >> 16) & 0xFF,
...
/* End BUSMASTER generated function - OnTimer_brakeTransmit_10 */