...
GCC_EXTERN void GCC_EXPORT OnTimer_SpeedTransmit_10();
...
bool InitUDP()
{
    ...
    Trace("Speed ECU: UDP socket bound to port 5100");
    return true;
}
...
void OnDLL_Load()
{
    ...
    if (!InitUDP()) {
        Trace("Speed ECU: ERROR - Failed to initialize UDP!");
    } else {
        Trace("Speed ECU: Ready, waiting for CARLA data on port 5100");
    }
}
...
/* Start BUSMASTER generated function - OnTimer_SpeedTransmit_10 */
void OnTimer_SpeedTransmit_10()
{
    ...
    if (received >= 4) {
        ...
        float speed = NetworkToHostFloat((unsigned char*)buffer);
        
        /* Set signal value - copy float bytes directly */
        UINT32 speedValue;
        memcpy(&speedValue, &speed, 4);
        speedMsg.speed_current_kmh = speedValue;
        
        /* Send CAN message */
        SendMsg(speedMsg);
        
        messageCount++;
        Trace("Speed ECU: RX %.2f km/h -> TX CAN ID=0x123 Data=[%02X %02X %02X %02X]", 
              speed,
              (speedValue) & 0xFF,
              (speedValue >> 8) & 0xFF,
              (speedValue >> 16) & 0xFF,
...
/* End BUSMASTER generated function - OnTimer_SpeedTransmit_10 */